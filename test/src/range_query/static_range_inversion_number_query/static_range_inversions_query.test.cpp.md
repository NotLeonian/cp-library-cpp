...
data:
  ...
  bundledCode: "#line 1 \"test/src/range_query/static_range_inversion_number_query/static_range_inversions_query.test.cpp\"\
    ...
    \        StaticRangeInversionNumberQuery(std::vector<T> a) : _block_size(std::max(1,\
    \ int(::sqrt(a.size())))), _block_num(ceil_div(a.size(),_block_size)) {\n    \
    ...
    \ i = 0; i < _block_size; ++i) _sorted[block_id][i] = { a[offset + i], offset\
    ...
    \         * @brief Calculates the inversion number of the subarray (a_l, ...,\
    \ a_{r-1}).\n         * @param l left (closed)\n         * @param r right (open)\n\
    \         * @return Inversion number of (a_l, ..., a_{r-1})\n         */\n   \
    \     long long operator()(int l, int r) const {\n            const int bl = l\
    \ / _block_size, br = ceil_div(r, _block_size);\n            // [bl][bl+1,...,br-2][br-1]\n\
    \            if (br - bl == 1) {\n                /**\n                 *    \
    \         l   r\n                 *         | A | B | C |\n                 *\
    \ Blocks: |           |\n                 *\n                 * Inv(B)=Inv(AB)+Inv(BC)-Inv(ABC)+Inv(A,C)\n\
    \                 */\n                long long res = _pinv[bl][r - bl * _block_size]\
    \ + _sinv[bl + 1][(bl + 1) * _block_size - l] - _pinv[bl][_block_size];\n    \
    ...
    \                 */\n                const int ml = (bl + 1) * _block_size, mr\
    \ = (br - 1) * _block_size;\n                long long res = _pinv[bl + 1][r -\
    \ ml] + _sinv[br - 1][mr - l] - _pinv[bl + 1][mr - ml];\n                for (int\
    \ k = _block_size - 1, j = _block_size, cnt = 0; k >= 0; --k) {\n            \
    ...
    \n        // _pinv[i][j] := inversion number of the subarray (a_{i * _block_size},\
    \ ..., a_{i * _block_size + j - 1})\n        // _sinv[i][j] := inversion number\
    \ of the subarray (a_{i * _block_size - j}, ..., a_{i * _block_size - 1})\n  \
    \      std::vector<std::vector<long long>> _pinv, _sinv;\n\n        // _sorted[i]\
    \ := sorted array of { (a_{i * _block_size + j}, i * _block_size + j) | 0 <= j\
    ...
    \                const int offset = block_id * _block_size;\n                pinv[block_id].resize(_block_size\
    ...
    \ _block_size * block_id\n                for (int i = _block_size * (_block_num\
    \ - 1 - block_id) - 1, j = _block_size; i >= 0; --i) {\n                    while\
    \ (j > 0 and cmp(suf[i].first, pre[j - 1].first)) --j;\n                    pinv[block_id][suf[i].second\
    \ - offset + 1] += _block_size - j;\n                }\n\n                for\
    \ (int i = _block_size; i < _block_size * (_block_num - block_id); ++i) {\n  \
    \                  pinv[block_id][i + 1] += pinv[block_id][i] + pinv[block_id\
    \ + 1][i + 1 - _block_size] - pinv[block_id + 1][i - _block_size];\n         \
    \       }\n\n                std::copy(pre.begin(), pre.end(), std::back_inserter(suf));\n\
    \                std::inplace_merge(suf.begin(), suf.end() - _block_size, suf.end(),\
    \ cmp_pair);\n            }\n            return pinv;\n        }\n\n        static\
