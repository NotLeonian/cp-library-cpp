...
data:
  ...
  bundledCode: "#line 1 \"test/src/datastructure/union_find/weighted_union_find/DSL_1_B.test.cpp\"\
    ...
    \            explicit WeightedUnionFind(int n) : n(n), par(n), component_size(n,\
    ...
    \ y) {\n                assert(same(x, y));\n                return op(neg(weight(x)),\
    \ weight(y));\n            }\n            // Check if `x` and `y` belongs to the\
    \ same connected component.\n            bool same(int x, int y) {\n         \
    \       return root(x) == root(y);\n            }\n            // Get the size\
    \ of connected component to which `x` belongs.\n            int size(int x) {\n\
