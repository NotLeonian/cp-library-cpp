...
data:
  ...
  bundledCode: "#line 1 \"test/src/polynomial/compose/composition_of_formal_power_series.test.cpp\"\
    ...
    \ fac(size_f), fac_inv(size_f);\n            fac[0] = 1;\n            for (int\
    \ i = 1; i <= size_f - 1; ++i) fac[i] = fac[i - 1] * i;\n            fac_inv[size_f\
    \ - 1] = fac[size_f - 1].inv();\n            for (int i = size_f - 1; i >= 1;\
    \ --i) fac_inv[i - 1] = fac_inv[i] * i;\n\n            std::vector<mint> ec(size_f),\
    \ fa(size_f);\n            mint p = 1;\n            for (int i = 0; i < size_f;\
    \ ++i, p *= a) {\n                ec[i] = p * fac_inv[i];\n                fa[size_f\
