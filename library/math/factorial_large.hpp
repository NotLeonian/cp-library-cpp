...
namespace suisen {
    ...
    struct factorial_large {
        ...
        static value_type perm(long long n, long long r) {
            if (r < 0 or r > n or n / mint::mod() != r / mint::mod()) return 0;
            return fac(n % mint::mod()) * fac_inv(n - r);
        }
        // perm(n, r) の逆数
        // perm(n, r) = 0 の場合は assert 違反となる
        static value_type perm_inv(long long n, long long r) {
            assert(r >= 0 and n >= r and n / mint::mod() == r / mint::mod());
            return fac_inv(n % mint::mod()) * fac(n - r);
        }
    private:
